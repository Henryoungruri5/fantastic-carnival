...
def build_ffmpeg_cmd(task: dict, v_path: str, s_path: str, out_path: str) -> tuple:
    ...
    if mode == 'gpu':
        ...
        if codec == 'h265':
            ...
            # ✅ FIX 1: -sn se soft subs strip hoti hain
...
async def cmd_htu(client, message):
    ...
    await message.reply_text(
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🖋️ **FONT**\n"
        "Koi bhi `.ttf` file bhejo → auto install + active!\n"
        "`/settings` → Font menu → change karo\n\n"

        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "⚙️ **SETTINGS**\n"
        "`/settings` — GPU/CPU codec, quality, resolution, threads, maxrate, tune\n"
        "GPU: H.264 NVENC / H.265 HEVC\n"
        "CPU: H.264 x264 / H.265 x265\n"
        ...
        "`/hsubm` — Batch HardSub\n"
        "`/end`   — Batch finalize\n"
        "`/changesub` — Subtitle change karo\n"
        "`/rename` — File rename\n"
        "`/extract` — Sub extract\n"
        "`/crtoass` `/ctosrt` `/ctovtt` — Convert\n"
        "`/status` `/cancel` `/settings`\n"
        "`/htu` — Yeh guide!"
    )
...
async def encode_single_task(client, task):
    ...
    status_msg = await client.send_message(
        chat_id,
        f"🔥 **Encoding** {'📦 Batch' if is_batch else '🎬 Single'} | "
