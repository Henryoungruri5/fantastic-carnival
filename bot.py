...
# 🎛️  UI CONSTANTS
# ════════════════════════════════════════════════════════════
RESOLUTIONS     = ['original', '1080p', '720p', '480p', '360p', '144p']
...
# 🔨  FFMPEG HELPERS
# ════════════════════════════════════════════════════════════
RESOLUTION_MAP = {'original': None, '1080p': '1920:1080', '720p': '1280:720', '480p': '854:480', '360p': '640:360', '144p': '256:144'}
...
async def cmd_htu(client, message):
    ...
    await message.reply_text(
        ...
        "`/c1080`, `/c720`, `/c480`, `/c360`, `/c144` → Kisi bhi video par reply karo (current CPU/GPU settings use hongi, bas resolution change hogi).\n"
        "💡 **Multiple videos:** Alag alag videos pe reply karke same command chalao — sab queue mein jud jayenge!\n\n"

        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🔇 **AUDIO REMOVE**\n"
        "`/AudioR` → Video par reply karo → saari audio tracks remove ho jayengi!\n"
        "Done hone ke baad naam choose karo (original ya custom) — exactly compress jaisa flow.\n\n"
...
async def cmd_start(client, message):
    ...
    await message.reply_text(
        ...
        f"`/c1080` `/c720` `/c480` `/c360` `/c144` — Convert resolution\n"
        f"`/AudioR` — Video se audio remove karo\n"
...
# ──────────────────────────────────────────────────────────
# ──────────────────────────────────────────────────────────
# 🔄  RESOLUTION CONVERT SHORTCUTS  (single reply OR multiple replies)
# ──────────────────────────────────────────────────────────
@app.on_message(filters.command(["c1080", "c720", "c480", "c360", "c144"]))
async def cmd_convert_res(client, message):
    if not is_admin(message.from_user.id): return

    res_map = {"c1080": "1080p", "c720": "720p", "c480": "480p", "c360": "360p", "c144": "144p"}
    ...
    # ── Collect video(s): reply message + any quoted/forwarded chain ──
    video_msgs = []
    if message.reply_to_message:
        ref = message.reply_to_message
        def _is_video(m):
            return m.video or (getattr(m.document, "file_name", "") or "").lower().endswith(('.mp4', '.mkv', '.avi', '.mov', '.ts'))
        if _is_video(ref):
            video_msgs.append(ref)

    if not video_msgs:
        await message.reply_text(
            f"❌ Kisi video par reply karke **/{cmd}** likho!\n\n"
            f"💡 **Multiple videos:** Ek ke baad ek reply karke /{cmd} use karo — sab queue mein jayenge."
        )
        return
    ...
    for video_msg in video_msgs:
        original_fname = (
            getattr(video_msg.video, "file_name", None) or
            getattr(video_msg.document, "file_name", None) or "video.mkv"
        )
    ...
    await message.reply_text(
        f"✅ **{added} video(s) queue mein add!** (pos #{pos})\n"
        f"⚙️ {d['mode'].upper()} [{codec_disp}] → {target_res}\n\n"
        f"💡 Aur videos? Unpe bhi reply karke `/{cmd}` karo — queue mein jud jayenge!"
    )
...
# ──────────────────────────────────────────────────────────
# 🔇  /AudioR — Remove ALL audio from video (reply to video)
# ──────────────────────────────────────────────────────────
@app.on_message(filters.command(["AudioR", "audior"]))
async def cmd_audio_remove(client, message):
    if not is_admin(message.from_user.id): return

    if not message.reply_to_message or not (
        message.reply_to_message.video or
        (getattr(message.reply_to_message.document, "file_name", "") or "").lower().endswith(('.mp4', '.mkv', '.avi', '.mov', '.ts'))
    ):
        await message.reply_text(
            "🔇 **Audio Remove**\n\n"
            "Kisi video par reply karke `/AudioR` likho!\n"
            "→ Video se saari audio tracks remove ho jayengi."
        )
        return
    ...
    task_id = next_tid()
    out_path = f"downloads/audioR_{chat_id}_{task_id}.mkv"
    ...
    v_path = None
    try:
        v_path = await video_msg.download(
            file_name=f"downloads/audioR_in_{chat_id}_{task_id}.tmp"
        )
        await status_msg.edit_text(
            f"🔇 **Audio Remove — Processing...**\n"
            f"📁 `{original_fname}`\n"
            f"⏳ FFmpeg chal raha hai... /cancel"
        )
        ...
        if proc.returncode == 0 and os.path.exists(out_path):
            # Ask for rename
            d = get_data(chat_id)
            orig_base = os.path.splitext(original_fname)[0]
            suggested = f"{orig_base}_NoAudio.mkv"
            d['status']     = 'waiting_rename'
            d['temp_file']  = out_path
            d['_last_orig'] = suggested
            await status_msg.edit_text(
                f"✅ **Audio Removed!**\n\n"
                f"📁 Original: `{original_fname}`\n"
                f"🔇 Audio tracks: sabhi remove\n\n"
                f"📝 Naam choose karein:",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("📁 Original Naam", callback_data="keep_original_name"),
                    InlineKeyboardButton("✏️ Custom Naam",   callback_data="prompt_rename"),
                ]])
            )
            return  # out_path ko cleanup mein delete mat karo
        else:
            await status_msg.edit_text(f"❌ **FFmpeg fail!** Task #{task_id}")
        ...
        # out_path sirf fail pe cleanup, success pe rename handler karega
        if os.path.exists(out_path) and get_data(chat_id).get('temp_file') != out_path:
...
HANDLED_CMDS = [
    ...
    "crtoass","ctosrt","ctovtt","hardsub","hsub","htu","changesub",
    "c1080", "c720", "c480", "c360", "c144",
    "AudioR", "audior"
]
